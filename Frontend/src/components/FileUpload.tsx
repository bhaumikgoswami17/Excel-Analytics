...
const FileUpload: React.FC<Props> = ({ onFileUpload, isLoading }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      await onFileUpload(selectedFile); // ✅ use the parent function
    } catch (err) {
      ...
      <div className="space-y-3">
        ...
        {selectedFile ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-600">.xls or .xlsx, max 10MB</p>
            <button
              onClick={() => inputRef.current?.click()}
              disabled={isLoading}
        ...
        {error && (
          <div className="mt-3 bg-red-50 border border-red-200 rounded-md p-2 text-sm text-red-700 flex items-start">
